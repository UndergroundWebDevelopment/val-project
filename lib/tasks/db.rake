namespace :db do
  ...
  task :seed_user, [:email, :password] => :environment do |t, args|
    raise "Email is required. Proper usage: `rake db:seed[email, password]`" unless args[:email]
    raise "Password is required. Proper usage: `rake db:seed[email, password]`" unless args[:password]

    password_hash = BCrypt::Password.create(args[:password])
    database.run("INSERT INTO users (email, password_hash) VALUES(#{database.literal(args[:email])}, #{database.literal(password_hash)});")
  end
